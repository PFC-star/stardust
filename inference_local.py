...
import time
from system_pipeline.onnx_backend.onnx import torch_to_onnx_residual, model_output_size, device_module_assignment, torch_to_onnx, initialize_onnx_model
...
def run_onnx_sequential_inference(onnx_model_paths, tokenizer, input_text, max_tokens=30, quantization_option=False):
    """
    Perform sequential inference with pre-split ONNX models.
    
    Args:
        onnx_model_paths: List of paths to ONNX model files in sequence order
        tokenizer: HuggingFace tokenizer for the model
        input_text: Input text for inference
        max_tokens: Maximum number of tokens to generate
        quantization_option: Whether to use quantization
    
    Returns:
        Generated text
    """
    import onnxruntime as ort
    from system_pipeline.onnx_backend.onnx import initialize_onnx_model
    
    # Create inference sessions for each model and load module names
    sessions = []
    module_names = []
    
    for idx, model_path in enumerate(onnx_model_paths):
        # Get the module directory and name file path
        module_dir = os.path.dirname(model_path)
        name_path = os.path.join(module_dir, f"module_name_{idx}.json")
        
        # Initialize ONNX model using the original function
        session, name = initialize_onnx_model(module_dir, name_path, idx, quantization_option=quantization_option)
    ...
    all_tokens = input_ids.numpy()
    begin_time = time.time()
    
    for i in range(max_tokens):
        # Forward through all models in sequence
        current_input = all_tokens
        
        for idx, (session, module_name) in enumerate(zip(sessions, module_names)):
            ...
            # Run inference
            if idx == 0:
                # First module takes the input directly
                outputs = session.run([output_name], {input_name: [current_input]})
            else:
                # Subsequent modules take the output from previous module
                outputs = session.run([output_name], {input_name: current_input})
            
            current_input = outputs[0]
            
            # For debugging
            next_token_scores = outputs[0][0, -1, :]
            next_token_id = np.argmax(next_token_scores)
            print(f'Inference at submodel_{idx} ({module_name}), potential next token: {next_token_id}')
        
        # Get next token from the final output
        next_token_scores = current_input[0, -1, :]
        next_token_id = np.argmax(next_token_scores)
        
        # Append the predicted token to the sequence
        next_token_array = np.array([[next_token_id]])
        all_tokens = np.concatenate((all_tokens, next_token_array), axis=1)
        
        # Print generated token
        print(f'Generated token: {next_token_id}, text so far: {tokenizer.decode(all_tokens[0])}')
    
    end_time = time.time()
    print(f"\nONNX sequential inference time: {end_time - begin_time}")
    
    # Decode the full sequence
    generated_text = tokenizer.decode(all_tokens[0])
...
if __name__ == "__main__":
    ...
    # model_output_size(tokenizer,
    #                   "/workspace/ams-LinguaLinked-Inference/onnx_model__/backup/bloom560m_quantized_int8_seq",
    #                   split_size,
    #                   quantization_option=True,
    #                   residual_connection=False,
    ...
    # Example usage of ONNX sequential inference
    # Uncomment and modify paths as needed

    model_name = "../LLM_models/bloom-560m"
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    onnx_model_paths = [
        "onnx_model__/backup/bloom560m_unquantized_seq/module0/module_0.onnx",
        "onnx_model__/backup/bloom560m_unquantized_seq/module1/module_1.onnx"
    ]

    generated_text = run_onnx_sequential_inference(
        onnx_model_paths,
        tokenizer,
        "什么是分布式大语言模型？",
        max_tokens=30
