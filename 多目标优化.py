...
def simulated_annealing(M_total, n, iterations=5000, initial_temp=100, cooling_rate=0.99):
    # 随机初始化子模型分配
    Mi = np.random.dirichlet(np.ones(n), size=1)[0] * M_total
    ...
    for iteration in range(iterations):
        # 随机生成新的分配方案
        new_Mi = np.random.dirichlet(np.ones(n), size=1)[0] * M_total
...
def calc_total_time(Mi):
    ...
    for i in range(1, n):
        ...
        # 计算当前设备的总时间：加载 + 推理 + 通信
        time_i = load_time(Mi[i]) + inference_time_i + comm_time
        total_times.append(time_i)
