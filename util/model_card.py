...
available_models = {
    "bloom560m": ["../LLM_models/bloom-560m", "huggingface_tokenizer"],
    # "bloom560m": ["bigscience/bloom-560m", "huggingface_tokenizer"],
    # "bloom1b1": ["bigscience/bloom-1b1", "huggingface_tokenizer"],
    "bloom1b1": ["/workspace/LLM_models/bloom-1b1", "huggingface_tokenizer"],
    "bloom1b7": ["szzzzz/chatbot_bloom_1b7", "huggingface_tokenizer"],
    # "bloom1b7": ["../LLM_models/chatbot_bloom_1b7", "huggingface_tokenizer"],
    "bloom3b": ["/workspace/LLM_models/bloom-3b", "huggingface_tokenizer"],
    "bloom7b": ["/workspace/LLM_models//bloom-7b1", "huggingface_tokenizer"],
     "llama8B": ["/workspace/LLM_models/llama8B", "huggingface_tokenizer"],
    "vicuna13B": ["../LLM_models/llava-v1.6-vicuna-13b-hf", "sentencepiece_tokenizer"],
    "Baichuan13B": ["../LLM_models/Baichuan-13B-Chat", "huggingface_tokenizer"],
    ...
    "qwen2_7b": ["../LLM_models/Qwen2-7B-Instruct", "huggingface_tokenizer"],
    "Qwen14B-Chat-Int4": ["/workspace/LLM_models//Qwen14B-Chat-Int4", "huggingface_tokenizer"],
...
class ModelCard:
    ...
    def load_model_and_tokenizer(self):
        ...
        if self.task_type == "Generation":
            ...
            if model_to_load.startswith("gptq"):
                ...
                self.model = AutoModelForCausalLM.from_pretrained(model_to_load, low_cpu_mem_usage=True, trust_remote_code=True)
    ...
    def get_project_directory(self):
        ...
        while True:
            ...
            if tail == "ams-LinguaLinked-Inference":  # the name of your project
                return current_path
            else:
                current_path = head
    ...
    def prepare_model_split(self):
        ...
        if self.split_size:
            ...
            split_size = max_split_size(self.model, transformer_model_option=self.transformer_model_option)
        sequential_dependency_map = None
        residual_dependency_map = None
        if self.residual_connection:
            modules, sequential_dependency_map, residual_dependency_map, checked_split_size = model_sharding(self.model,
                                                                                                             split_size,
                                                                                                             transformer_model_option=self.transformer_model_option,
                                                                                                             residual_connection=self.residual_connection,
                                                                                                             debug_mod=True)
            self.module_flop_map = get_module_flops(modules, self.tokenizer,
                                                    sequential_dependency_map, residual_dependency_map)
            # self.module_flop_map = {"bloom":560}
            self.sequential_dependency_map = sequential_dependency_map
            self.residual_dependency_map = residual_dependency_map
        else:
            modules, checked_split_size = model_sharding(self.model,
                                                         split_size,
                                                         transformer_model_option=self.transformer_model_option,
                                                         residual_connection=self.residual_connection,
                                                         debug_mod=False)
        ...
        onnx_model_directory = os.path.join(project_level_directory, 'onnx_model__')
        ...
        input_for_export = self.tokenizer("This is a test input for exporting model", return_tensors="pt")["input_ids"]
        ...
        if not os.listdir(onnx_module_path):
            if self.residual_connection and self.split_size >= 2:
                if self.sequential_dependency_map is not None and self.residual_dependency_map is not None:
                    torch_to_onnx_residual(module_list=modules,
                                           input=input_for_export,
                                           export_path=onnx_module_path,
                                           sequential_dependency_map=self.sequential_dependency_map,
                                           residual_dependency_map=self.residual_dependency_map,
    ...
    def generate_cpu_answer(self, prompt, num_beams = 5, max_new_tokens = 100):
        start = time.time()
        ans = self.tokenizer.decode(
            self.model.generate(**self.tokenizer(prompt, return_tensors="pt").to("cpu"), num_beams=num_beams, max_new_tokens=max_new_tokens)[
...
def retrieve_sending_info(root_dir, model_name, ip_module_list, quantization_option, residual_connection):
    ...
    directory_path = os.path.join(root_dir, 'onnx_model__', 'backup', quantized_option_path)
...
def retrieve_sending_dir(root_dir, model_name, quantization_option, residual_connection):
    ...
    directory_path = os.path.join(root_dir, 'onnx_model__', 'to_send', quantized_option_path)
